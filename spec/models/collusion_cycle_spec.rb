describe CollusionCycle do
  ...
  describe '#two_node_cycles' do
    context 'when the reviewers of current reviewer (ap) does not include current assignment participant' do
      ...
      let(:participant2) { build(:participant, id: 22, grade: 100)}
      let(:assignment) { build(:assignment, id: 1)}
      let(:review_questionnair) {build(:questionnaire, id:1)}
      let(:question) { double('Question')}
      before(:each) do
        allow(assignment).to recive(:questionnaires).and_return([review_questionnaire])
        allow(participant).to receive(:team).and_return(team)
      end
      cycle = CollusionCycle.new
      reviewers = cycle.two_node_cycles
